...
while True: # Infinite loop until broken by user action
    ...
    if user_answer.lower() == 'n': # If user is installing CaRAS in the base environment (where Anaconda 3 is installed)
...
if user_answer.lower() == 'n': # Execute this if user is installing CaRAS in the base environment (where Anaconda 3 is installed)
...
if sys.platform == "linux" or sys.platform == "linux2":  # Execute this instead if the installation platform is Linux
    ...
    subprocess.run('unzip -o {}/gem_macOS.zip'.format(caras_env_dir), shell = True)
