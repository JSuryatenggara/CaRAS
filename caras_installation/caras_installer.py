...
while True: # Infinite loop until broken by user action
    user_answer = input('Do you want to install CaRAS in a specific environment? (Y/N) ') # Proceed only after the user provided Y or N answer to this question

    if user_answer.lower() == 'y': # If user is installing CaRAS in a newly created environment
        user_environment = input('Please type in the name of your CaRAS environment: ') # Ask what the user wants the newly created conda environment to be named with
        caras_env_dir = '{}/envs/{}'.format(conda_dir, user_environment)
        prefix_string = 'prefix: {}'.format(caras_env_dir) # Path to the directory in which the newly created environment components are installed
        name_string = 'name: {}'.format(user_environment) # The name of the newly created environment
        input('CaRAS will be installed in {} environment. Press ENTER to continue'.format(user_environment)) # Notify the user
        break # Dialogue finished. Proceed with the installation.

    if user_answer.lower() == 'n': # If user is installing CaRAS in the base environment (where Anaconda / Miniconda / Miniforge is installed)
        user_environment = 'base' # The default name of the environment where Anaconda / Miniconda / Miniforge is installed
        caras_env_dir = '{}'.format(conda_dir)
        prefix_string = 'prefix: {}'.format(caras_env_dir) # Path to the directory in which the newly created environment components are installed
        name_string = 'name: {}'.format(user_environment) # The name of the newly created environment
        input('CaRAS will be installed in base environment. Press ENTER to continue') # Notify the user
        break # Dialogue finished. Proceed with the installation.
...
if sys.platform == "linux" or sys.platform == "linux2": # Execute this if the installation platform is Linux
    shutil.copy('{}/caras_env_linux.yml'.format(sys.path[0]), '{}/caras_{}.yml'.format(sys.path[0], user_environment)) # Copy paste the provided caras_env_linux.yml file
    with open('{}/caras_{}.yml'.format(sys.path[0], user_environment), 'r') as original_yml: yml_contents = original_yml.readlines() # Read the copy pasted file contents as list of lines
    ...
    shutil.copy('{}/caras_env_macos.yml'.format(sys.path[0]), '{}/caras_{}.yml'.format(sys.path[0], user_environment)) # Copy paste the provided caras_env_macos.yml file
    with open('{}/caras_{}.yml'.format(sys.path[0], user_environment), 'r') as original_yml: yml_contents = original_yml.readlines() # Read the copy pasted file contents as list of lines
...
if user_answer.lower() == 'y': # Execute this if user is installing CaRAS in a newly created environment
    if sys.platform == "linux" or sys.platform == "linux2":
        subprocess.run('conda env create -f {}/caras_{}.yml'.format(sys.path[0], user_environment), shell = True)
    if sys.platform == "darwin":
        subprocess.run('conda env create -f {}/caras_{}.yml --platform osx-64'.format(sys.path[0], user_environment), shell = True)
        subprocess.run('conda config --env --name {} --set subdir osx-64'.format(sys.path[0], user_environment), shell = True)
        subprocess.run('CONDA_PREFIX={} conda config --set subdir osx-64'.format(caras_env_dir), shell = True)

if user_answer.lower() == 'n': # Execute this if user is installing CaRAS in the base environment (where Anaconda / Miniconda / Miniforge is installed)
    subprocess.run('conda env update -f {}/caras_{}.yml'.format(sys.path[0], user_environment), shell = True)
...
if os.path.isfile('{}/caras_{}.yml'.format(sys.path[0], user_environment)):
    os.remove('{}/caras_{}.yml'.format(sys.path[0], user_environment))


subprocess.run('chmod +x {}/caras_scripts/*'.format(sys.path[0]), shell = True) # Mark all scripts in folder "caras_scripts" as executable
...
# Find every instance of idr.py script in the CaRAS installation conda environment and put them all into a list
popen_idr = subprocess.Popen('find {} -name "idr.py"'.format(caras_env_dir), shell = True, stdout = subprocess.PIPE)
...
for idr_full_path in idr_full_path_list: # For every instance of idr.py script found above
    print("Replacing installed stock IDR script ({}) with CaRAS's patched IDR script ({}/idr.py)".format(idr_full_path, sys.path[0]))
    shutil.copy('{}/idr.py'.format(sys.path[0]), idr_full_path) # Replace the "broken" stock IDR script with the patched one provided by CaRAS installation package
...
hg38_homer_genome_command_line = 'perl {}/share/homer*/.//configureHomer.pl -install hg38'.format(caras_env_dir) # Define the new path to homer based on installation directory
hg19_homer_genome_command_line = 'perl {}/share/homer*/.//configureHomer.pl -install hg19'.format(caras_env_dir) # Define the new path to homer based on installation directory
mm9_homer_genome_command_line = 'perl {}/share/homer*/.//configureHomer.pl -install mm9'.format(caras_env_dir) # Define the new path to homer based on installation directory
mm10_homer_genome_command_line = 'perl {}/share/homer*/.//configureHomer.pl -install mm10'.format(caras_env_dir) # Define the new path to homer based on installation directory
sacCer3_homer_genome_command_line = 'perl {}/share/homer*/.//configureHomer.pl -install sacCer3'.format(caras_env_dir) # Define the new path to homer based on installation directory
dm6_homer_genome_command_line = 'perl {}/share/homer*/.//configureHomer.pl -install dm6'.format(caras_env_dir) # Define the new path to homer based on installation directory
...
path_to_caras_scripts = 'PATH=$PATH:{}/caras_scripts'.format(sys.path[0]) # Define the path to the directory "caras_scripts"
...
if sys.platform == "linux" or sys.platform == "linux2":  # Execute this instead if the installation platform is Linux
    ...
    with open(os.path.expanduser('~/.bashrc'), "r+") as file:
        for line in file:
            if '{}\n'.format(path_to_caras_scripts) == line: # Look for the path to the directory "caras_scripts"
        ...
        if path_exist == 0: # If it is not found until the end of the lines
            file.write('{}\n'.format(path_to_caras_scripts)) # Write the path to the directory "caras_scripts" so the scripts within can be accessed from any directory
    subprocess.run('exec bash', shell = True) # reload bashrc


elif sys.platform == "darwin":  # Execute this instead if the installation platform is MacOS
    subprocess.run('cp ./gem_macOS.zip {}/'.format(caras_env_dir), shell = True)
    # shutil.copy('./gem_macOS.zip'.format("./",caras_env_dir))
    ...
    if os.path.isfile('~/.bash_profile'): # If the computer already has the .bash_profile file
        ...
        with open(os.path.expanduser('~/.bash_profile'), "r+") as file: # Open the .bash_profile file
            ...
            if path_exist == 0: # If it is not found until the end of the lines     
                cwd = os.getcwd()
                file.write('export PATH="{}/caras_scripts:$PATH"'.format(cwd)) # Write the path to the directory "caras_scripts" so the scripts within can be accessed from any directory
